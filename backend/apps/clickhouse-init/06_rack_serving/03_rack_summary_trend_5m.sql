-- =========================================================
-- FILE NAME: 03_rack_summary_trend_5m.sql
-- MUC DICH:
--   Tao view xu huong trung han 5 phut cho Rack tu aggregate 5m cua Node.
--   View nay phuc vu chart 24h, 7d va phan tich degradation dai hon
--   o cap Rack theo huong operator-first.
-- TANG DU LIEU PHUC VU:
--   06_rack_serving / mid-term trend / rack 5m trend
-- PHU THUOC:
...
--   - Trend 5m giam nhieu hon 1m va phu hop cho 24h, 7d.
--   - Trend 5m khong suy dien stale/unknown theo current-time nhu node_current_live.
-- =========================================================

USE telemetry_db;

-- Xoa view cu neu ton tai de tao lai theo version modular.
DROP VIEW IF EXISTS telemetry_db.rack_summary_trend_5m;

-- View tong hop xu huong Rack theo bucket 5 phut.
CREATE VIEW telemetry_db.rack_summary_trend_5m AS
SELECT
    bucket_start,
    rack_id,
...
(
    ...
    -- trong moi bucket 5 phut cua rack.
    ...
    (
        -- =================================================
        -- TANG metric bucket 5m:
        -- Doc aggregate 5m cua node, tra cuu policy dong tu
...
-- =========================================================
-- TEST GOI Y
-- Muc dich: kiem tra view rack_summary_trend_5m cho chart trung han,
-- culprit trend, va blast radius trend o cap Rack.
-- =========================================================

-- Test 1:
-- Xem trend 5m cua tat ca rack trong 24 gio gan nhat
-- SELECT
--     bucket_start,
--     rack_id,
--     rack_severity_code,
...
--     worst_node_id,
--     worst_metric_key
-- FROM telemetry_db.rack_summary_trend_5m
-- WHERE bucket_start >= now() - INTERVAL 24 HOUR
-- ORDER BY bucket_start, rack_id;

-- Test 2:
-- Xem trend 5m cua mot rack cu the
-- SELECT
--     bucket_start,
--     rack_id,
--     rack_severity_code,
