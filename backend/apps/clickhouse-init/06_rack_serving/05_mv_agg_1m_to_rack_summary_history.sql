-- =========================================================
-- FILE NAME: 05_mv_agg_1m_to_rack_summary_history.sql
-- MUC DICH:
--   Materialize xu huong Rack 1 phut vao bang rack_summary_history
--   ma khong can query-time roll-up lai tu node-level raw.
-- TANG DU LIEU PHUC VU:
--   06_rack_serving / historical rack semantics / 1m materialization
-- PHU THUOC:
...
-- DAU RA CHINH:
--   - telemetry_db.mv_agg_1m_to_rack_summary_history
-- GHI CHU:
--   - Materialized view nay doc tu bang aggregate 1m co insert that.
--   - Pipeline van giu 2 buoc:
--       (1) metric-series bucket -> node bucket
--       (2) node bucket -> rack bucket
--   - Khong suy dien stale/unknown cho lich su 1m.
-- =========================================================

USE telemetry_db;

-- Xoa MV cu neu ton tai de tao lai theo version modular.
DROP VIEW IF EXISTS telemetry_db.mv_agg_1m_to_rack_summary_history;

-- MV day du lieu 1m vao bang rack_summary_history.
CREATE MATERIALIZED VIEW IF NOT EXISTS telemetry_db.mv_agg_1m_to_rack_summary_history
...
SELECT
    ...
    bucket_start,
    rack_id,
    -- Ghi aggregate states de merge duoc khi cung bucket nhan nhieu dot insert.
...
(
    -- =====================================================
    -- BƯỚC 1:
    -- Chuan hoa source aggregate 1m thanh 1 dong moi node trong rack
    -- cho moi bucket 1 phut.
    -- =====================================================
    SELECT
        bucket_start,
        rack_id,
    ...
    (
        -- =================================================
        -- Tang metric bucket 1m da duoc finalize tu aggregate state.
        ...
        (
            -- =============================================
            -- Finalize cac state columns cua agg_1m de lay
