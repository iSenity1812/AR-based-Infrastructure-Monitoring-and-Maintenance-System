-- =========================================================
-- FILE NAME: 10_service_summary_trend_5m.sql
-- MUC DICH:
--   Tao view xu huong trung han 5 phut cho Service theo mo hinh
--   hybrid:
--   - cluster-state native tu aggregate 5m cua service.*
--   - blast radius va culprit tu container_summary_trend_5m
-- TANG DU LIEU PHUC VU:
--   07_container_service_serving / mid-term trend / service 5m trend
-- PHU THUOC:
...
-- GHI CHU:
--   - Trend 5m khong suy dien stale/unknown theo current-time.
...
DROP VIEW IF EXISTS telemetry_db.service_summary_trend_5m;

CREATE VIEW telemetry_db.service_summary_trend_5m AS
WITH
all_service_bucket_ids AS (
    ...
    (
        ...
        FROM telemetry_db.container_summary_trend_5m
        WHERE service_name != ''
        GROUP BY
            bucket_start,
            service_id
        ...
        (
            ...
            (
                SELECT
                    ...
                    src.value_last,
                    src.text_last
                FROM telemetry_db.v_agg_5m_by_scope_metric AS src
                LEFT JOIN
                (
                    SELECT
                        bucket_start,
