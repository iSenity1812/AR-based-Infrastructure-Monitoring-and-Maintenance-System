-- =========================================================
-- FILE NAME: 13_mv_agg_5m_to_service_summary_history.sql
-- MUC DICH:
--   Materialize service_summary_trend_5m vao bang service_summary_history
--   de backend co the query lich su service 5 phut truc tiep.
-- TANG DU LIEU PHUC VU:
--   07_container_service_serving / historical service semantics / 5m
-- PHU THUOC:
--   - telemetry_db.agg_5m_by_scope_metric
--   - telemetry_db.service_summary_trend_5m
--   - telemetry_db.service_summary_history
-- DAU RA CHINH:
--   - telemetry_db.mv_agg_5m_to_service_summary_history
-- GHI CHU:
--   - Pattern giong ban 1m nhung trigger source la aggregate 5m.
--   - Phu hop cho lich su 24h-7d cua service ma khong can roll-up lai.
-- =========================================================

USE telemetry_db;

-- Xoa MV cu neu ton tai de tao lai theo version modular.
DROP VIEW IF EXISTS telemetry_db.mv_agg_5m_to_service_summary_history;

-- MV day du lieu 5m vao bang service_summary_history.
CREATE MATERIALIZED VIEW IF NOT EXISTS telemetry_db.mv_agg_5m_to_service_summary_history
TO telemetry_db.service_summary_history
AS
WITH changed_buckets AS
(
    -- Bucket nao trong aggregate 5m duoc cap nhat thi can re-materialize.
...
(
    ...
    FROM telemetry_db.service_summary_trend_5m
) AS src
